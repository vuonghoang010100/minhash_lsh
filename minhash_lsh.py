...
# declare hash_function used in minhash function
hash_fn = [make_random_hash_fn() for i in range(0, NUM_HASH)]
# ---------------------------------------------------------------------------------------

def get_signature(num_hash, shingled_data):
    '''
    '''
...
def minhash(shingled_data, doc_id , num_hash):
    ...
    # for each document
    for id in doc_id:
        # get signatures of current document
        signatures = get_signature(num_hash, shingled_data[id])
        signature_matrix.append(signatures)
...
if __name__ == "__main__":  
    # check input value
    if len(sys.argv) != 2:
        print("Invalid input\ntry: $python3 minhash_lsh_p_process.py <number of documents>")
        exit()
    ...
    print("Prossing %d document:" % document_size)
